...
> 存在的问题:\
> 如果授权后，发现将服务器时间早于当前时间该怎么处理？\
> 目前是继续运行。
> 授权一个月，授权成功后，关机或关闭程序，一个月后再运行，如果时间慢了，理论上过期了，但还能使用，这情况无法处理。只能使用 时钟锁

## 加密算法
> odin与edda 的部分常量，用户密码，哈希盐，加密算法，在对接中要约定好。\
> 0.9.3版本 之后odin不再实现加密算法，算法由使用者实现后传给odin,odin只负责回调加密算法。\
> github.com/offer365/example/endecrypt 下提供了 aes,des,rsa,ecc 加密算法,\
> github.com/offer365/example/tools 下提供了若干hash算法,\
> 序列号，授权码可以采用 rsa2048+aes256加密或 ecc+aes256加密\
> Auth步骤 请求中Verify字段，可以使用，aes256加密，\
> Cipher使用rsa1024 \
> Auth使用rsa2048 \
